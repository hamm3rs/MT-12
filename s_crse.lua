...
local function run(power, switch, deadzo) 						-- Number of params must match number of params in the input table
	...
	if switch <= 0 then		-- changed to <= from == so that L## switch can be used.	-- Are we in "neutral" mode > set all to zero and back again
		cruise_value = 0
		prev_power = 0
		return 0, 0
	end
	...
	local cruise_mode = 1
	--commented out to use <= L## above.  so that cruise can stay on without holding down a button.
	--Negative curise is disable / not useful for me.
